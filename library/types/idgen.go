...
func InitIDGen(seed string) bool {
	...
	if err != nil {
		nodeSN = 0
		logger.Alert("wrong config in seed,Atoi fail:%v,err:%v", seed, err.Error())
		return false
	}
	if idint > 0x3FFF || idint <= 0 {
		nodeSN = 0
		logger.Alert("wrong config in seed range:%v", seed)
		return false
	}
...
func NewObjectID() (id ObjectID) {
	if nodeSN == 0 {
		logger.Error("something wrong in idgen seed")
		return 0
	}
